...
namespace AdventureGame
{
    ...
    public class Adventure
    {
        ...
        { 
            ...
            rm9list.Add(new Thing("scrap of paper", "Someone has written a message on the scrap of paper using a blunt pentil. It says 'This space is intentionally left black'" ));
            ...
            kennelcontents.Add(new Thing("wombat", "It's a cuddly little wombat. It is squeaking gentle to itself."));

            ThingList rm12list = new ThingList();
            rm12list.Add(new Thing("sign", "The sign says: Do not feed the wombat!", false));
            ThingHolder kennel = new ThingHolder("kennel", "This kennel is about the right size for a wombat!", false, kennelcontents);
            rm12list.Add(kennel);

            ThingList rm13list = new ThingList();
            rm13list.Add(new Thing("toad", "It is a slimy toad that keeps saying 'greep-greep'"));
            ...
            /* 3 */
            _map.Add(new Room("Grassy Hill", "A grassy mound. You can see a forest stretching away to the South.", 1, NOEXIT, 1, 4, new ThingList()));
            /* 4 */
            _map.Add(new Room("Cave", "A dark, dripping cave.", 2, NOEXIT, 3, 5, new ThingList()));
            /* 5 */
            ...
            /* 7 */
            _map.Add(new Room("Passage", "A maze of twisty little passage, all alike.", 7, 8, 5, 8, new ThingList()));
            /* 8 */
            _map.Add(new Room("Passage", "A maze of twisty little passage, all alike.", 7,6,6,7, new ThingList()));
            /* 9 */
        ...
        {
            string s = "";
            // return string msg to display after attempting moveTo() operation
        ...
        public string TakeOb(string obname)
        {
            ...
            if( obname == "")
            {
                obname = "nameless object"; // if no object sepcified
            }
            if (t == null)
            {
        ...
        // Let each object respond with any special actions rather than use this method
        private string DropObSpecialAction(Thing t, Room r)
        {
            string s = "";
            if ((t.Name == "carrot") && (r.Name == "Kennel Room"))
            {
                s = "If you want wombat to come out, you'll have to do some coding first!";
            }
            return s;
        }
